...
// Allow same-origin embedding only: the datapot.org / datapot.net main site
// frames this app from its own domain (we live at <domain>/drop).
...
// Link generation runs on a small shared server — set expectations by size.
function estWait(bytes) {
  const mb = bytes / 1048576;
  if (mb < 10)  return 'a few seconds';
  if (mb < 100) return 'under a minute';
  return '1–2 minutes';
}
...
function setFile(f) {
  ...
  document.querySelector('.fhint').textContent =
    'after upload, generating the link takes about ' + estWait(f.size);
  hideError();
}
...
$('form').addEventListener('submit', e => {
  ...
  xhr.upload.addEventListener('progress', ev => {
    ...
    if (ev.loaded >= ev.total) {
      ...
      $('pct').textContent = 'Upload complete — generating your link, this takes about '
                           + estWait(ev.total) + ' on our small server…';
      $('pbytes').textContent = '';
...
$('again').addEventListener('click', () => {
  ...
  document.querySelector('.fhint').textContent = 'click or drag & drop — max 300 MB';
